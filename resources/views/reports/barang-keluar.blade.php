...
@section('page-subtitle', 'Rekapitulasi pengeluaran barang inventory')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Barang Keluar</h1>
            <p class="mt-1 text-sm text-slate-500">Laporan detail distribusi dan pemakaian barang.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Total Transaksi -->
        <div class="relative overflow-hidden rounded-2xl bg-white p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-rose-100 group">
            ...
            <div class="relative flex items-center gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-rose-100 text-rose-600 group-hover:scale-110 transition-transform duration-300">
        ...
        <!-- Total Item -->
        <div class="relative overflow-hidden rounded-2xl bg-white p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-amber-100 group">
            ...
            <div class="relative flex items-center gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-amber-100 text-amber-600 group-hover:scale-110 transition-transform duration-300">
                    <i class="bi bi-layers text-xl"></i>
                </div>
                <div>
                    <p class="text-sm font-medium text-slate-500">Total Unit Keluar</p>
    ...
    <div class="rounded-3xl bg-white shadow-[0_20px_50px_rgba(8,_112,_184,_0.07)] ring-1 ring-slate-100/50 overflow-hidden">
        ...
        <div class="border-b border-slate-100 p-6 bg-slate-50/50">
            <form action="{{ route('reports.barang-keluar') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Mulai Tanggal</label>
                    <input type="date" name="start_date" class="block w-full rounded-xl border-slate-200 bg-white py-2 px-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 transition-all duration-200" value="{{ request('start_date') }}">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Sampai Tanggal</label>
                    <input type="date" name="end_date" class="block w-full rounded-xl border-slate-200 bg-white py-2 px-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 transition-all duration-200" value="{{ request('end_date') }}">
                </div>
                <div>
                    <button class="inline-flex w-full items-center justify-center rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-rose-200 transition-all duration-200 hover:bg-rose-700 hover:shadow-rose-300 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2">
        ...
        @if($transactions->count())
            <div class="overflow-x-auto">
                <table class="min-w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-rose-50/20 text-slate-500">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Tujuan/Keterangan</th>
                            <th scope="col" class="px-6 py-4 font-bold uppercase text-[11px] tracking-wider text-right">Jumlah</th>
                            <th scope="col" class="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Petugas</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($transactions as $t)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span class="inline-flex items-center rounded-lg bg-rose-50 px-2.5 py-1 text-xs font-bold text-rose-700 ring-1 ring-inset ring-rose-600/20">
